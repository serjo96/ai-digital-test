...
Gate использует заранее зафиксированные `sampleProductIds`: нужно полностью проверить все claims как минимум 20 выбранных карточек, а не все 158 claims. Файл всё равно обязан точно покрывать опубликованные claims по `productId + attempt + claimId` и совпадать с `publicationHash`. Текущая каноническая разметка — `generated-review-e478435a3d39.json`: 75/158 claims, 18/37 карточек и 18/20 карточек обязательной выборки. Осталось проверить Onyx Lite microSD и Quill 3 USB-C hub и самостоятельно пересмотреть шесть factual verdict, расходящихся с verifier; код их не исправляет от имени человека.
