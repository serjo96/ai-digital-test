...
Порядок проверки: сверить каждую строку и полноту семейства, исправить expectedGroups/nonProductRowIds/unknownPairs в labels.json. Разные группы и разные случаи означают известное различие, кроме явно указанных unknownPairs. При неизвестной связи не ставить отрицательную метку. Подтвердить весь случай только после проверки всех его отношений: status=human_verified, reviewedBy и reviewedAt (ISO дата). Никто ещё не подтверждал эти метки.

Holdout не оценивать и не использовать для настройки правил до этапа 5. Просмотр при составлении разметки уже выполнен; выборка не слепая для автора. После правки меток создать новую версию и новые отчёты; B0 не перезаписывать.
...
Development B3 создаёт рядом с run артефакт `generated-review.json`. Для человеческой проверки реально сгенерированного текста заполните rationale каждого опубликованного claim, исправьте `expectedVerdict` при необходимости, затем аналогично выставьте human metadata. Файл должен покрывать все опубликованные claims и совпадать с `publicationHash`; иначе pipeline его отклонит.
