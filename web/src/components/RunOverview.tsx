...
export function RunOverview({ catalog }: { catalog: CatalogSnapshot }) {
  const reviewProducts = catalog.products.filter(product => product.reviewIds.length > 0 || (catalog.listings[product.id]?.reviewFlags.length ?? 0) > 0).length;
  const outcomes = ['grouped', 'non_product', 'review'] as const;
  return <section className="run-overview" aria-label="Run overview">
    ...
    <p>{catalog.products.length} products · {reviewProducts} with review flags · {catalog.rows.length} source rows</p>
    <p>Development evaluation: {catalog.provenance?.qualityStatus ?? 'not supplied'}. Holdout not evaluated. Generation and claim verification have not run; no publication-ready listings.</p>
    <details>
      <summary>All row outcomes: {outcomes.map(outcome => `${outcome}: ${catalog.rows.filter(row => row.outcome === outcome).length}`).join(' · ')}</summary>
      <ul className="outcome-list">{catalog.rows.map(row => <li key={row.source.row_id}>
    ...
    <details>
      ...
      {catalog.rows.filter(row => row.outcome === 'non_product').map(row => <div className="source-card" key={row.source.row_id}>
