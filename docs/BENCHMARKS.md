...
Документ содержит накопительную историю: ранние блоки описывают состояние соответствующего этапа и не заменяют текущий статус. Актуальная передача находится в [NEXT_STEPS.md](NEXT_STEPS.md). Первичные метрики каждого запуска сохраняются в его `report.json`/`metrics.json`; экспортированные JSONL-серии нужны для сравнений и графиков.
...
P0.3 добавил явный split и один раз раскрыл holdout через offline full-input replay. [Holdout report](../reports/B3-openai-full-input-atomic-v2-holdout-replay/report.md): 6 cases / 49 rows; TP/FP/FN 22/0/0; precision, recall и candidate recall 22/22; true negatives 1154/1154; hard-negative specificity 256/256; non-product correctness 49/49; unknown/unevaluated pairs 0/0. Runtime: 0 API calls, 0 tokens, $0, 321 успешный cache hit. Эти показатели остаются provisional family-label метриками.

13 сентября пользователь завершил отдельный компактный matching audit: 20/20 items reviewed, 18 scored, 18 agreements, 0 disagreements и 2 `unknown`. Поэтому agreement = 18/18 = 1.0, scored coverage = 18/20 = 0.9. Development: 12/12 scored agreements и 2 unknown; holdout: 6/6 scored agreements. Экспорт прошёл schema/hash/row-ID/metadata validation, но пока находится вне репозитория; до его канонического сохранения эти числа ещё не считаются repo-backed benchmark. Выборка создана после раскрытия holdout и должна называться post-holdout validation, не независимым holdout.
