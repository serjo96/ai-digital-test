...
Updated: 2026-09-14. Status: stages 1–5 and the minimal stage 6 architecture stabilization are complete; B1-v2 remains accepted. Stage 3 was a completed experiment with a negative B2 decision: local Ollama extraction/shadow matching did not pass the quality gate. A later OpenAI development-only extraction validation also remained below the existing acceptance criteria and did not change B1 decisions; shadow matching was not enabled in that run. A separate B3 uses OpenAI only for publication generation and claim verification on top of ready B1 supports. P0.1/P0.2 produced a human-verified development gate: controlled 12/12, generated sample 20/20, factual/non-atomic errors 0/0. Full-input B3 is complete: 154/156 ready, 2 identity review with source-grounded unpublished drafts, 0 technically withheld, 213 covered rows and 390 published atomic claims; offline replay preserves the historical verified publication. Holdout replay was performed without network access. A compact post-holdout matching audit is saved in the repository: 20/20 reviewed, 18/18 agreement, 18/20 scored coverage, 2 unknown, 0 disagreements ([matching-audit-metrics.json](../eval/matching-audit-metrics.json)). Details: [ARCHITECTURE.md](ARCHITECTURE.md); [STAGE4_REPORT.md](STAGE4_REPORT.md); [STAGE3_REPORT.md](STAGE3_REPORT.md); [STAGE5_REPORT.md](STAGE5_REPORT.md); [metrics history](BENCHMARKS.md).
...
| 6. Minimal architecture stabilization | Explicit run-service boundaries, thin orchestration, centralized immutable storage | Completed required results from stages 4–5 | Completed; larger contract/ports migration deliberately deferred |
...
**Resolved debt:** `PipelineService` is now a dispatcher. B0/B1, B2, B3, and comparison orchestration live in separate services; run I/O is centralized in `RunStoreService`; AI provider registration lives in `AiModule`; and `AppModule` is only a composition root. Domain types and functions stayed at their compatible paths.

**Implemented boundaries:**

1. `PipelineService.run` selects `CatalogRunService`, `AiMatchingRunService`, or `PublicationRunService`; comparison has its own service.
2. `RunStoreService` owns run reservation, validated reads, and exclusive writes. A second storage interface is not introduced without a second implementation.
3. `AiModule` registers the existing provider registry and adapters. AI contracts, prompts, thresholds, and cache format are unchanged.
4. Architecture tests protect the composition root, dispatcher, framework-free domain, current web integration surface, compatibility exports, and runtime import graph.
5. Characterization tests freeze B0/B1 decisions and the B3 publication hash. The full-input replay reproduces 321 cached jobs with zero calls.

**Completion criterion:** 87 backend and 18 web tests, typecheck, and production build pass; B1 `decisionsHash` and B3 replay `publicationHash` match the fixed references; CLI commands and fail-closed gates remain compatible. Automated verification makes no live model calls; a separate explicitly authorized development integration check exercised OpenAI live and replay paths. Explicit accounting-regression, storage-error, and architecture-boundary tests are included. Backend coverage remains at or above the pre-refactor 92.27% lines / 85.02% branches.

**Deliberate cuts:** no HTTP API, database/queue, full ports-and-adapters layer, browser contracts package, historical-report cleanup, or speculative runtime-scaling work. These remain possible later changes when a real transport, storage, deployment, or volume requirement exists.

## 7. Metrics and comparison protocol

### What is currently known

220 rows; 5 suppliers; 12 categories; 23 empty raw_specs; 4 explicitly problematic rows; 169 titles after trim + casefold. These are input statistics, not ground truth. B1-v2: 220/220, 156 products; development TP/FP/FN 17/0/0 provisional on extended family labels. The B2 quality gate was not accepted, so B3 is built on B1-v2. Current OpenAI B3 development gate: controlled human-verified 12/12; generated review v2 human-verified 76/99 claims, sample 20/20. Full-input B3: 154/156 ready, 2 identity review with unpublished drafts, 390 published claims. Holdout replay was saved without API calls. Expanded matching labels cover 108 rows / 70 groups and remain provisional. A separate post-holdout audit is saved in the repository: 20/20 reviewed, 18/18 agreement, 18/20 scored coverage, 2 unknown, 0 disagreements ([matching-audit-metrics.json](../eval/matching-audit-metrics.json)). Artifacts: [STAGE5_REPORT.md](STAGE5_REPORT.md), [STAGE4_REPORT.md](STAGE4_REPORT.md), [BENCHMARKS.md](BENCHMARKS.md).
...
**Addendum 2026-09-14:** B3 listing/claim review remains complete and stored in artifacts; the final UI shows Catalog + one compact Review. The user confirmed controlled 12/12, generated sample 20/20, and answered 20 atomic matching questions. The export and metric are saved as `eval/matching-audit-human-verified.json` and `eval/matching-audit-metrics.json`: 20/20 reviewed, 18/18 agreement, 18/20 scored coverage, 2 unknown, 0 disagreements (post-holdout validation). The minimal stage 6 stabilization is complete without changing product rules, prompts, labels, or historical artifacts; holdout has already been revealed.
