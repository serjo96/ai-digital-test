...
Текущий блокер следующего этапа: завершить и принять generated human review; controlled suite из 12 случаев пользователь подтвердил 2026-09-11. До успешного generated gate full-input B3, holdout и финальная оценка не разрешены. Исторически `web:prepare` отклонял schema 4/B3; это ограничение снято отдельным заданием этапа 5, описанным ниже.
...
Черновик generated verdict/rationale хранится только в `localStorage` с привязкой к `publicationHash`; экспорт остаётся provisional до принятия generated review. Исходный run не изменяется, модели и сеть из UI не вызываются. Проверены реальный development bundle, обе вкладки и восстановление черновика после reload. Все 12 controlled cases пользователь подтвердил; full-input B3, holdout, финальная оценка и deployment не выполнялись, generated human gate остаётся блокером.
...
Канонический файл `eval/generated-review-e478435a3d39.json` сохраняет все 75 решений пользователя и добавляет в обязательную выборку Onyx Lite microSD и Quill 3 USB-C hub: текущее состояние 75/158 claims, 18/37 карточек, sample 18/20. Старый localStorage переносится по `productId + attempt + claimId`, но не может затереть repository-backed reviewed decisions. `web:prepare --generated-checks` подключает этот файл без перезаписи исторического run. Full-input, holdout, replay, API, deployment, commit и push в P0.1 не выполнялись.
