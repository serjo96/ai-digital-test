...
Для завершения этапа 5 пользователь должен ответить на 20 атомарных вопросов и передать экспорт. Затем сохраняется отдельная matching audit-метрика и выполняется clean-clone проверка. Расширенные 20 family cases (108 строк / 70 групп) остаются provisional и не повышаются от имени пользователя. Поскольку holdout уже раскрыт, компактная выборка обозначается post-holdout validation. Deployment, commit и push не выполнялись.
