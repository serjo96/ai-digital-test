...
Updated: 2026-09-13. B1-v2 remains the accepted product baseline. **The measured B2 was an experiment using only local Ollama models** and did not pass the quality gate; the prepared OpenAI B2 profiles were not run, so OpenAI B2 quality was not measured. **B3 is a separate OpenAI publication stage**, not a continuation of B2: `gpt-5.6-sol` formulates text from already accepted B1 supports, and a separate `gpt-6-astra` verifies claims. Controlled and generated development gates are human-verified; full-input replay is preserved. The compact matching audit is saved in the repository ([matching-audit-metrics.json](eval/matching-audit-metrics.json)): 20/20 reviewed, 18/18 agreement, 18/20 scored coverage, 2 `unknown`, 0 disagreements. The expanded family labels remain provisional. Stages 1–5 are complete; stage 6 is optional and not started.
...
| B2 | Experimental extraction and shadow matching | Local Ollama `qwen3:4b` / `gemma4:12b`; OpenAI live calls = 0 | No, the quality gate was not accepted |
...
|---|---|---|---|---|---|
| Semantic extraction (B2, local development) | Five additional attributes with exact citations | Ollama qwen3:4b / gemma4:12b; OpenAI live was not run | Selecting the meaning and subject in fragments the rules do not understand | Wrong subject, number, or scope; schema, local checks, and review | 12 development rows per model; live latency in the stage 3 report; local cost N/A |
| Type / category (within extraction) | Propose a type and one of 12 categories from evidence | Same local model, same extraction request | Semantic wording missed by rules | An accessory treated as a device; a confident code decision is not overwritten | No additional calls; usage/latency in ai.json |
| Matching (B2, strictly shadow) | Preserve merge/reject/unknown, confidence, reason, and evidence for 8 identical pairs | Local Ollama qwen3:4b / gemma4:12b; OpenAI live was not run | Comparing the meaning of names and conditions | False merge; the recommendation does not change decisions, groups, or product confidence at all | 2 positive, 4 negative, 2 unknown; results and latency in the stage 3 report |
| Description generation (B3 development) | Short neutral text based only on conflict-free identity and agreed product facts | OpenAI `gpt-5.6-sol`, low | Formulating coherent text | Fabricated claim; text is not published without separate complete verification | 37 calls; 28582 tokens; $0.1119028; median/p95 1.937/2.954 s |
...
Why a model: a fragment requires selecting the meaning, subject, and applicable attribute, not merely finding a number. Code keeps narrow, verifiable boundaries for accepting the result. This remains a hypothesis about AI usefulness, not a demonstrated gain over several additional rules. If the live experiment does not demonstrate value, B1-v2 remains accepted.
